...
import { FaEnvelope, FaLock, FaEye, FaEyeSlash, FaGoogle, FaSuitcaseRolling, FaPassport, FaCamera, FaCompass, FaFacebook, FaApple, FaMoon, FaSun } from 'react-icons/fa';
...
const Login = () => {
    ...
    // 🌟 ස්වයංක්‍රීයව Current Host එක හඳුනාගෙන API URL එක සකස් කිරීම (localhost හෝ වෙනත් ඕනෑම Wi-Fi IP එකකට වැඩ කරයි)
        ...
        if (validate()) {
            try {
                ...
                if (response.ok && data.token) {
                    ...
                    // 🌟 ලොග් වෙන යූසර්ගේ Email එක සේව් කරගන්නවා
                    localStorage.setItem('userEmail', data.user.email);

                    // 🌟 අදාළ Email එකට මීට කලින් ලකුණු සේව් වෙලා නැත්නම් විතරක් 0 කරනවා
                    ...
                    if (data.user.role === 'admin') {
                        navigate('/loading?to=/admin/dashboard');
                    } else {
    ...
    // 🌟 Google Button එක ක්ලික් කරද්දී Modal එක පෙන්වන Logic එක
                ...
                callback: async (response) => {
                    ...
                    try {
                        ...
                        if (backendRes.ok && backendData.token) {
                            ...
                            // 🌟 Google Login එකෙන් එන යූසර්ගේ Email එක සේව් කිරීම
                            localStorage.setItem('userEmail', userData.email);

                            // 🌟 Google යූසර්ගේ Email එකට අදාළව ලකුණු Key එක සකස් කිරීම
                            ...
                            if (backendData.user?.role === 'admin') {
                                setTimeout(() => navigate('/loading?to=/admin/dashboard'), 800);
                            } else {
                                setTimeout(() => navigate('/loading?to=/home'), 800);
                            }
                        } else {
                            alert(backendData.message || backendData.error || 'Backend synchronization failed');
                        }
                    } catch (err) {
    ...
    const handleGoogleSelect = (account) => {
        ...
        // 🌟 Saved account එකකින් සිලෙක්ට් කරද්දීත් අදාළ Email එක අප්ඩේට් කිරීම
