...
const DATABASE_CATEGORIES = ["All", "Sri Lankan", "Indian", "Chinese", "Fast Food"];

const TRANSLATIONS = {
  English: {
    heading: "Discover Best Food Hubs",
    subtitle: "Find the best restaurants, cafes, and dining spots near you.",
    sidebarTitle: "Locations",
    allDistricts: "All Districts",
    searchLocationPlh: "Search district or city...",
    searchPackagePlh: "Search foods or restaurants...",
    perPerson: "/ avg price",
    categories: ["All", "Restaurants", "Cafes", "Street Food", "Pubs & Bars"]
  },
  Sinhala: {
    heading: "පරිපූර්ණ රසවත් ආහාර ස්ථාන",
    subtitle: "ඔබට සමීපව ඇති හොඳම අවන්හල්, කැෆේ සහ ආහාර රසවිඳිය හැකි ස්ථාන සොයා ගන්න.",
    sidebarTitle: "ස්ථාන",
    allDistricts: "සියලු දිස්ත්‍රික්ක",
    searchLocationPlh: "දිස්ත්‍රික්කය හෝ නගරය සොයන්න...",
    searchPackagePlh: "ආහාර හෝ අවන්හල් සොයන්න...",
    perPerson: "/ සාමාන්‍ය මිල",
    categories: ["සියල්ල", "අවන්හල්", "කැෆේ", "වීදි ආහාර", "පබ් සහ බාර්"]
  },
  Tamil: {
    heading: "சிறந்த உணவு மையங்கள்",
    subtitle: "உங்களுக்கு அருகிலுள்ள சிறந்த உணவகங்கள், கஃபேக்கள் மற்றும் உணவு இடங்களைக் கண்டறியுங்கள்.",
    sidebarTitle: "இடங்கள்",
    allDistricts: "அனைத்து மாவட்டங்கள்",
    searchLocationPlh: "மாவட்டம் அல்லது நகரம் தேடு...",
    searchPackagePlh: "உணவு அல்லது உணவகங்களைத் தேடு...",
    perPerson: "/ சராசரி விலை",
    categories: ["அனைத்தும்", "உணவகங்கள்", "கஃபேக்கள்", "தெரு உணவு", "பப்கள் மற்றும் பார்கள்"]
  }
};
...
const FoodHub = () => {
  ...
  const [formData, setFormData] = useState({ restaurantName: '', city: '', district: '', restaurantCategory: '', contact: '', image: '' });
  ...
  const fetchRestaurantsData = async () => {
    try {
      ...
      const params = { category: 'food-hub', isApproved: 'true' };

      if (currentSubCategory !== 'All') params.subCategory = currentSubCategory;
      if (foodSearch.trim() !== '') params.search = foodSearch;
      ...
      setRestaurants(response.data.success ? response.data.data : response.data);
    } catch (error) { console.error("Error loading:", error); }
  };

  useEffect(() => { fetchRestaurantsData(); }, [selectedType, selectedName, activeCategory, foodSearch]);
    ...
    <div className="min-h-screen text-white pt-6 pb-12 px-4 md:px-8 max-w-7xl mx-auto">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
            <motion.form initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              onSubmit={async (e) => {
                ...
                const payload = {
                  ...
                  category: 'food-hub',
                  subCategory: formData.restaurantCategory,
                ...
                try {
                  ...
                  await axios.post(`${API_BASE_URL}/api/restaurants`, payload, {
                  ...
                  setFormData({ restaurantName: '', city: '', district: '', restaurantCategory: '', contact: '', image: '' });
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <h2 className="text-xl font-bold text-orange-500">➕ Add New Restaurant</h2>
                <FaTimes className="cursor-pointer text-gray-400" onClick={() => setIsModalOpen(false)} />
              </div>

              <input required placeholder="Restaurant Name" className="w-full bg-[#161922] p-3 rounded-xl border border-gray-700 text-sm" value={formData.restaurantName} onChange={e => setFormData({...formData, restaurantName: e.target.value})} />
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <select required disabled={modalCities.length === 0} className="w-full bg-[#161922] p-3 rounded-xl border border-gray-700 text-xs disabled:opacity-40" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})}>
                  <option value="">City</option>
                  {modalCities.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>

              <select required className="w-full bg-[#161922] p-3 rounded-xl border border-gray-700 text-sm" value={formData.restaurantCategory} onChange={e => setFormData({...formData, restaurantCategory: e.target.value})}>
      ...
      <div className="flex flex-col lg:flex-row gap-8 mt-10">
        <div className="w-full lg:w-64 flex flex-col gap-4">
          ...
          <button onClick={() => setIsModalOpen(true)} className="w-full bg-emerald-600 py-3 rounded-2xl font-bold text-xs flex justify-center items-center gap-2"><FaPlusCircle /> ADD RESTAURANT</button>
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between mt-6 mb-8">
            <input type="text" placeholder={text.searchPackagePlh} value={foodSearch} onChange={(e) => setFoodSearch(e.target.value)} className="w-full sm:w-72 bg-[#11131a] p-2 rounded-xl border border-gray-800 text-sm" />
          ...
          {selectedName !== 'All Districts' ? (
            <div className="flex flex-col gap-4">
              {restaurants.length === 0 ? (
                <p className="text-gray-400 text-sm">No restaurants found for this category and location.</p>
              ) : (
                restaurants.map((pkg, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                      ...
                      <button
                        onClick={() => alert(`About ${pkg.name}: Located in ${pkg.location}. Category: ${pkg.subCategory || 'Restaurant'}`)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {restaurants.length === 0 ? (
                <p className="text-gray-400 text-sm col-span-full text-center py-10">No restaurants found for this category.</p>
              ) : (
                restaurants.map((pkg, index) => (
                  <motion.div key={pkg._id || index} className="bg-[#11131a] rounded-2xl overflow-hidden border border-gray-800 flex flex-col justify-between">
                    <img src={pkg.image || 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=500'} alt={pkg.name} className="w-full h-44 object-cover" />
