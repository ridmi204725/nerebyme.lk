...
import {
    FaMapMarkerAlt, FaRoute, FaStar, FaPhoneAlt,
...
export default function DetailsTravel() {
    ...
    const [subPart, setSubPart] = useState('travel');
    ...
    // Login වී ඇති User ගේ විස්තර නිවැරදිව ලබා ගැනීම
    ...
    // Backend එකෙන් අදාළ Travel විස්තර සහ Reviews ලබාගැනීම
    useEffect(() => {
        try {
        ...
        const fetchDetails = async () => {
            try {
                ...
                if (res.data && res.data.data) {
                    ...
                    setSubPart(res.data.data.category || 'travel');
            ...
            {itemDetails ? (
                <div className="w-full bg-gray-900 border border-gray-800 rounded-3xl shadow-2xl overflow-hidden">
                    ...
                    <div className="relative h-72 sm:h-96 md:h-[450px] w-full">
                        <img
                            src={itemDetails.image || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=1200'}
                        ...
                        <div className="absolute bottom-6 left-6 right-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                            <div>
                                <span className="bg-emerald-600 text-white text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider mb-2 inline-block">
                                ...
                                <p className="text-xs sm:text-sm text-gray-300 flex items-center gap-1.5 mt-1">
                                    <FaMapMarkerAlt className="text-emerald-400" size={14} /> {itemDetails.location || 'Location not specified'}
                                </p>
                            </div>
                    ...
                    <div className="p-4 sm:p-6 md:p-10 space-y-8">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            <div className="bg-gray-950 border border-gray-800 p-4 rounded-2xl flex items-center gap-3">
                                <div className="p-3 bg-emerald-600/10 text-emerald-400 rounded-xl border border-emerald-500/20"><FaCoins size={18} /></div>
                                <div>
                                    <p className="text-[11px] text-gray-400 font-medium">Travel Package Price</p>
                                    <p className="text-sm font-bold text-white">Rs. {itemDetails.price || itemDetails.pricePerNight || 'N/A'}</p>
                                </div>
                            </div>

                            <div className="bg-gray-950 border border-gray-800 p-4 rounded-2xl flex items-center gap-3">
                                <div className="p-3 bg-blue-600/10 text-blue-400 rounded-xl border border-blue-500/20"><FaPhoneAlt size={18} /></div>
                                <div>
                            ...
                            <div className="bg-gray-950 border border-gray-800 p-4 rounded-2xl flex items-center justify-between sm:col-span-2 md:col-span-1">
                                ...
                                <a
                                    ...
                                    className="bg-emerald-600 hover:bg-emerald-500 text-white px-3.5 py-2 rounded-xl text-xs font-bold transition shadow-md"
                                >
                                    Open Map
                                </a>
                            </div>
                        ...
                        <div className="bg-gray-950/60 border border-gray-800 p-6 rounded-2xl space-y-3">
                            <h2 className="text-base font-bold text-emerald-400 flex items-center gap-2">
                                <FaInfoCircle size={16} /> About This Travel Destination
                            </h2>
                        ...
                        {/* Packages & Itinerary / Facilities */}
                        <div className="space-y-4">
                            <h2 className="text-base font-bold text-emerald-400 flex items-center gap-2">
                                <FaLayerGroup size={16} /> Travel Packages & Itinerary
                            </h2>
                            {Array.isArray(itemDetails.menu) && itemDetails.menu.length > 0 ? (
                                    ...
                                    {itemDetails.menu.map((pkg, idx) => (
                                        <div key={idx} className="bg-gray-950 border border-gray-800 p-4 rounded-2xl space-y-2">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <span className="text-xs bg-emerald-600/20 text-emerald-300 px-2 py-0.5 rounded-md font-semibold">{pkg.category || 'Tour Plan'}</span>
                                            </div>
                                ...
                                <div className="bg-gray-950 border border-gray-800 p-4 rounded-2xl">
                                    <p className="text-xs font-semibold text-gray-300">Tour Highlights & Features:</p>
                        ...
                        <div className="space-y-6">
                            <div className="bg-gray-950 border border-gray-800 p-5 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-bold text-white">Traveler Experiences</h3>
                                        <p className="text-xs text-gray-400 mt-0.5">Share your feedback with other travelers.</p>
                                    </div>
                                </div>
                                ...
                                <button
                                    ...
                                    className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold cursor-pointer transition shadow-lg"
                                >
                            ...
                            {/* Review Form */}
                            {showReviewForm && (
                                <form onSubmit={handleReviewSubmit} className="bg-gray-950 border border-emerald-500/30 p-6 rounded-2xl space-y-4 animate-fadeIn">
                                    <h3 className="text-sm font-bold text-emerald-400">Add Your Review</h3>

                                    <div className="flex items-center gap-3 bg-gray-900 p-3 rounded-xl border border-gray-800">
                                        <FaUserCircle className="text-emerald-400 text-2xl" />
                                        <div>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-bold text-emerald-400 ml-2">({ratingValue} / 5 Stars)</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Write about your travel experience here..."
                                            className="w-full bg-gray-900 border border-gray-800 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-emerald-500 transition"
                                            required
                                        ></textarea>
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer"
                                    >
                ...
                <div className="w-full bg-gray-900 border border-gray-800 p-12 rounded-3xl text-center space-y-4">
                    <p className="text-gray-400 text-sm">No travel record selected.</p>
                    <button
                        onClick={() => navigate('/travel')}
                        className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 px-6 rounded-xl text-xs transition cursor-pointer"
                    >
                        Go Back to Travel Places
                    </button>
                </div>
            )}
        </div>
